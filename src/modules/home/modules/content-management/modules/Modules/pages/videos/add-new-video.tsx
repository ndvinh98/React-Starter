...
function AddNew() {
  ...
  const {getList: getListProduct, data: productsData, setData: setProductsData} =
    ...
    const {getList: getListModule, data: moduleData, setData: setModuleData} =
  ...
  const {data: dataUploadVideo, getItem: getVideo} = useGetItem<{url: string; value: string}>(
    'productModuleResources/uploadFileUrl',
  );

  const {data: dataDownloadVideo, getItem: getDownloadVideo} = useGetItem<{url: string; value: string}>(
  ...
  useEffect(() => {
    if (dataUploadVideo) {
      uploadFile(dataUploadVideo.url, fileVideo)
        .then(() => getDownloadVideo({name: JSON.stringify([dataUploadVideo.value])}))
  ...
  const canCreate =
    name && product && chooseLanguage && module && thumb && dataUploadVideo?.value;
  ...
  return (
    <UI.Box py={5} px={7}>
      ...
      <UI.VStack
        ...
        <UI.HStack w="full">
          <UI.Text w="300px">Video Name</UI.Text>
