...
function Applications() {
  ...
  const {
    ...
    getItem,
    loading: itemLoading,
  } = useGetItem(`groupings/${params?.productGroupId}`);
  const [page, setPage] = useState(1);
  const [limit, setLimit] = useState(10);
  const [layout, setLayout] = useState<'grid' | 'list'>('grid');

  useEffect(() => {
    if (params && params?.productGroupId) {
      getItem({
        partnerId: params?.partnerId,
      });
      getList({
        filter: JSON.stringify([{grouping: params.productGroupId}]),
        partnerId: params?.partnerId,
        page: page,
        limit: limit,
        cache: true,
  ...
  }, [params?.productGroupId, page, limit]);
